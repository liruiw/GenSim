...
class SimulationRunner:
    ...
    def save_stats(self):
        ...
        save_stat(self.cfg, self.cfg['model_output_dir'], self.generated_tasks, self.syntax_pass_rate / (self.curr_trials+1),
        ...
        try:
            ...
            if env_success_cnt >= total_cnt / 2:
                ...
                if  self.cfg['save_data']:
                    ...
                    save_task_flag = self.critic.reflection(self.generated_task, self.generated_code, current_tasks=self.generated_tasks)
                    if save_task_flag:
                        """ add to online buffer """
                        self.memory.save_task_to_online(self.generated_task, self.generated_code)
                        print(f"added new task to online buffer: {self.generated_task['task-name']}")
                        self.passed_tasks.append(self.generated_task['task-name'])

                        if self.cfg['save_memory']:
                            """ add to disk for future inspection and use """
                            self.memory.save_task_to_offline(self.generated_task, self.generated_code)
